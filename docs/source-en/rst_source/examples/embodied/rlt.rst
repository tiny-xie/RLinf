...
Record MLP actor segments separately
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

``env.train.rlt_actor_data_collection`` can sidecar-export consecutive action
chunks actually generated by the Stage 2 MLP as a separate LeRobot dataset.
The regular ``env.train.data_collection`` continues to record the original
rollout and the replay buffer is unchanged. Segment boundaries use the
rollout's ``actor_switch`` provenance, so the already-generated reference chunk
in which ``b`` is pressed is not mislabeled as an MLP action.
...
The two ``save_dir`` values must differ. ``only_success=False`` retains MLP
segments from both successful and failed rollouts; set it to ``True`` to retain
